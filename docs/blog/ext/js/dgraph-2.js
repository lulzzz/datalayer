...
var DGraph2 = function() {
	function t() {
		DGraph2.Util.runIfClassNamePresent("page-home", e)
	}
	function e() {
		DGraph2.Util.isMobile ? DGraph2.Home.mobileHero() : DGraph2.Nodes.init()
	}
	return {
		initialize : t
	}
...
!function() {
	...
	DGraph2.Util = {}, DGraph2.Util.isMobile = e, DGraph2.Util.runIfClassNamePresent = t
}();
...
!function() {
	...
	DGraph2.Home = {}, DGraph2.Home.mobileHero = t
}();
...
!function() {
	...
	function s() {
		var t = document.getElementById("node-canvas-2");
	...
	var l = 1400, h = 490, u = 50, c = 128, d = 0, f = [];
	for (i = 0; c > i; i++)
		f.push({
	...
	var p = d3.layout.force().size([ l, h ]).nodes(f).linkDistance(60).charge(
			-1).gravity(4e-4).on("tick", e), g = d3.select("#dgraph-2")
			.append("svg").attr("id", "node-canvas-2").attr("width", l).attr(
					"height", h);
	...
	g.append("circle").attr("r", 30).attr("transform", "translate(-100,-100)")
		...
		s()
	}, DGraph2.Nodes = {}, DGraph2.Nodes.init = a
}();
