...
# Temporary disabeld waiting on support for spark.kubernetes.driver.pod.name property.
...
sed -e "s|SPARK_KUBERNETES_RESOURCESTAGINGSERVER_URI|$RESOURCESTAGINGSERVER_IP|g" /opt/zeppelin/conf/interpreter.json.template > /opt/zeppelin/conf/interpreter.json
